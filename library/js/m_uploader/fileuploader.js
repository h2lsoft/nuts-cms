...
qq.FileUploader = function(o){
    this._options = {
        ...
        template: '<div class="qq-uploader">' + 
				...
				'<div class="qq-upload-button"><input type="button"  value="'+nuts_lang_msg_69+'"></div>' +
				'<div class="qq-upload-cancelAll"><a href="javascript:;" onclick="$(\'.qq-upload-list li\').remove();">'+nuts_lang_msg_70+'</a></div>'+
        ...
        fileTemplate: '<li>' +
                ...
                '<a class="qq-upload-cancel" href="#">'+nuts_lang_msg_17+'</a>' +
                '<span class="qq-upload-failed-text">'+nuts_lang_msg_71+'</span>' +
            '</li>',
		...
		messagesFR: {
            //serverError: "Some files were not uploaded, please contact support and/or try again.",
            typeError: "`{file}` extension non valide. Seulement `{extensions}` sont autorisées.",
            sizeError: "`{file}` est trop lourd, maximum autorisée `{sizeLimit}`.",
            emptyError: "`{file}` est vide, merci de selectioner vos fichiers."
        },
...
qq.FileUploader.prototype = {
    ...
    _error: function(code, fileName){
		...
		if(nutsUserLang == 'fr')
			var message = this._options.messagesFR[code];
...
qq.UploadHandlerXhr.prototype = {
    ...
    upload: function(id, params){
        ...
        // build query string
        var queryString = '&lang='+nutsUserLang+'&qqfile=' + encodeURIComponent(name);
        for (var key in params){
            queryString += '&' + key + '=' + encodeURIComponent(params[key]);
        }
