...
function tpln_auto_security($value, $urldecode_before=false, $sanitize=true, $strip_tags_allowed='')
{
	...
	{
		$oldstring = $value;
		$value = preg_replace('#</*(script|embed|object|frame|frameset|ilayer|layer|bgsound|title|base)[^>]*>#i', "", $value);
	}
...
// code from bitlux
	...
	public function xssProtect($string)
	{
		...
		$string = str_ireplace(array("<?php", "<?", "<%", "?>", '<script>', '</script>'), '', $string);
		...
		{
			$oldstring = $string;
			$string = preg_replace('#</*(script|embed|object|frame|frameset|ilayer|layer|bgsound|title|base)[^>]*>#i', "</ xss >", $string);
		}
