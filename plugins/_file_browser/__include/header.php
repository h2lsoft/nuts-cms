...
function show_header($title) {			// header for html-page
	...
	if(count($titleX) >= 2)
	{
		$title_format = '<a href="index.php"><img align="absbottom" border="0" src="_img/_home.gif" /></a> ';
		$dirs = explode('/', $titleX[1]);
		...
		foreach($dirs as $dirX)
		{
			...
			$uri = str_replace('{dir}', $dir, 'index.php?action=list&dir={dir}&order=name&srt=yes#{dir}');
			$title_format .= '/<a href="'.$uri.'">'.$dirX.'</a> ';
		}
