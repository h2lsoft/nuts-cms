...
else
{
    ...
    {
        $cur_folder = str_replace(WEBSITE_PATH, '', $selected_path);
        $rigth_files_read = edmUserHasRight('READ', $cur_folder);

        $dirs2 = array();
        foreach($dirs as $key => $value)
        {
            $folderX = $selected_path.$key;
            $folderX = str_replace(WEBSITE_PATH, '', $folderX);
            if($value == "folder")
            {
                if(edmUserHasRight('LIST', $folderX))
                {
                    $dirs2[$key] = 'folder';
                }
            }
            elseif($rigth_files_read)
            {
                $dirs2[$key] = $value;
            }
        }
